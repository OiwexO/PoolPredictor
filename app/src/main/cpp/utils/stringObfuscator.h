...
time. OBFUSCATE returns a reference to an ay::obfuscated_string object with the
following traits:
	...
	- Global lifetime
	The actual instantiation of the ay::obfuscated_string takes place inside a
...
namespace ay {
    ...
    // Obfuscates or deobfuscates string with key
    constexpr void cipher(char* string, size_type size, key_type key) {
        // Obfuscate with a simple XOR cipher based on key
        for (size_type i = 0; i < size; i++) {
            string[i] ^= char((key >> ((i % 8) * 8)) & 0xFF);
        }
    }
    ...
    class obfuscator {
    public:
        // Obfuscates the string 'string' on construction
        constexpr obfuscator(const char* string) {
            // Copy string
    ...
    public:
        obfuscated_string(const obfuscator<N, KEY>& obfuscator) {
            // Copy obfuscated string
            for (size_type i = 0; i < N; i++) {
                m_string[i] = obfuscator.string()[i];
            }
        }

        ~obfuscated_string() {
            // Zero m_string to remove it from memory
            for (size_type i = 0; i < N; i++) {
                m_string[i] = 0;
            }
        }
    ...
    private:
        ...
        // Whether string is currently encrypted
    ...
    constexpr auto make_obfuscator(const char(&string)[N]) {
        return obfuscator<N, KEY>(string);
    }
}
...
// char*
#define AY_OBFUSCATE_KEY(string, key) \
	[]() -> ay::obfuscated_string<sizeof(string)/sizeof(string[0]), key>& { \
		static_assert(sizeof(decltype(key)) == sizeof(ay::key_type), "key must be a 64 bit unsigned integer"); \
		static_assert((key) >= (1ull << 56), "key must span all 8 bytes"); \
		constexpr auto n = sizeof(string)/sizeof(string[0]); \
		constexpr auto obfuscator = ay::make_obfuscator<n, key>(string); \
		thread_local auto obfuscated_string = ay::obfuscated_string<n, key>(obfuscator); \
